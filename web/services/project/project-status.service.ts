...
export class ProjectStatusService extends APIService {
  constructor() {
    super(API_BASE_URL);
  }
  ...
  async markDefault(workspaceSlug: string, projectId: string, statesId: string): Promise<void> {
    return this.post(`/api/workspaces/${workspaceSlug}/projects/${projectId}/states/${statesId}/mark-default/`, {})
  ...
  async getState(workspaceSlug: string, projectId: string): Promise<IState[]> {
  ...
  async getStatus(workspaceSlug: string, projectId: string, stateId: string): Promise<any> {
